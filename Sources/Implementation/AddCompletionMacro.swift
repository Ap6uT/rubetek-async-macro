...
public struct AddCompletionHandlerMacro: PeerMacro {
    public static func expansion<Context: MacroExpansionContext, Declaration: DeclSyntaxProtocol>(
        ...
        guard var funcDecl = declaration.as(FunctionDeclSyntax.self) else {
            throw MacroError.message("@addCompletionHandler only works on functions")
        }
        ...
        if funcDecl.signature.effectSpecifiers?.asyncSpecifier == nil {
            ...
            let diag = Diagnostic(
                ...
                message: SimpleDiagnosticMessage(message: "can only add a completion-handler variant to an 'async' function",
                                                 diagnosticID: messageID,
                                                 severity: .error),
                fixIts: [
                    // Fix-It to replace the '+' with a '-'.
                    FixIt(message: SimpleDiagnosticMessage(message: "add 'async'",
                                                           diagnosticID: messageID,
                                                           severity: .error),
                                                            changes: [
                                                                FixIt.Change.replace(oldNode: Syntax(funcDecl.signature),
        ...
        let completionHandlerParam = FunctionParameterSyntax(firstName: .identifier("completionHandler"),
