...
public struct AddAsyncMacro: PeerMacro {
    public static func expansion<Context: MacroExpansionContext, Declaration: DeclSyntaxProtocol>(
        ...
        if funcDecl.signature.effectSpecifiers?.asyncSpecifier != nil {
            throw MacroError.message("@addAsync requires an non async function")
        }

        // This only makes sense void functions
        guard funcDecl.signature.returnClause?.type.as(IdentifierTypeSyntax.self)?.name.text == "Void" || funcDecl.signature.returnClause == nil else {
            throw MacroError.message("@addAsync requires an function that returns void")
        }
        ...
        guard let completionHandlerParameterAttribute = funcDecl.signature.parameterClause.parameters.last?.type.as(AttributedTypeSyntax.self),
            ...
            throw MacroError.message("@addAsync requires an function that has a completion handler as last parameter")
        }
        ...
        if completionHandlerParameter.returnClause.type.as(IdentifierTypeSyntax.self)?.name.text != "Void" {
            throw MacroError.message("@addAsync requires an function that has a completion handler that returns Void")
        }
        ...
        let newAttributeList = funcDecl.attributes.filter {
            guard case let .attribute(attribute) = $0,
                  ...
                  let nodeType = node.attributeName.as(IdentifierTypeSyntax.self) else {
                return true
            }
    ...
    // add async
        let throwsSpecifier: TokenSyntax? = isResultReturn ? try? TokenSyntax(validating: .keyword(.throws)) : nil
        funcDecl.signature.effectSpecifiers = FunctionEffectSpecifiersSyntax(leadingTrivia: .space, asyncSpecifier: .keyword(.async), throwsSpecifier: throwsSpecifier)
        ...
        funcDecl.body = CodeBlockSyntax(leftBrace: .leftBraceToken(leadingTrivia: .space),
                                        statements: CodeBlockItemListSyntax([CodeBlockItemSyntax(item: .expr(newBody))]),
